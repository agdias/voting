		...
		if election_form.is_valid():
			...
			if utils.urlencode(election_params['short_name']) == election_params['short_name']:
				...
				try:
					...
					error = f"Já existe uma eleição com o nome abreviado {election_params['short_name']}"
			else:
				error = "Nenhum caracter especial é permitido no nome abreviado."
	...
	if request.method == "GET":
		...
		if election_form.is_valid():
			...
			try:
				...
				error = f"Já existe uma eleição com o nome abreviado {clean_data['short_name']}"
...
def one_election_extend(request, election):
	if request.method == "GET":
		...
		if election_form.is_valid():
			...
			# verifica se data/hora de extensão é posterior à data original de fim da eleição
			if (extension_date is not None) and (extension_date <= election.voting_ends_at):
				election_form.add_error('voting_extended_until', 'A data/hora deve ser posterior ao fim da eleição')
			else:
...
def one_election(request, election):
	...
	# se a eleição não foi iniciada, somente o administrador pode ver a cédula
	user = get_user(request)
...
def one_election_view(request, election):
	...
	# result!
	if election.result:
		status_update_message = "Os resultados estão em %s" % election.name
...
def trustee_login(request, election_short_name, trustee_email, trustee_secret):
	...
	if election:
		...
		raise Http404("Integrante da Comissão Eleitoral não reconhecido.")
	raise Http404(f"Eleição {election_short_name} não encontrada.")
...
def trustee_upload_pk(request, election, trustee):
	if request.method == "POST":
		...
		try:
			election.admin.send_message(f"{election.name} - integrante da Comissão Eleitoral envio chave privada" % election.name,
										f"integrante da Comissão Eleitoral {trustee.name} ({trustee.email}) enviou uma chave privada." % (trustee.name, trustee.email))
		except:
...
def one_election_cast_confirm(request, election):
	...
	# restrição para poder limitar o número de votos de cada eleitor na eleição
	if (voter is not None) and (election.cast_limit > 0) and (len(CastVote.get_by_voter(voter)) >= election.cast_limit):
		return render_template(request, 'generic_info', {
			'election': election,
			'info_title': 'O número máximo de cédulas já foi depositado',
			'info_content': 'Você não pode votar novamente nesta eleição'
		})
	...
	if request.method == "GET":
		...
		if voter and voter.can_update_status():
			status_update_label = voter.user.update_status_template() % "seu rastreador de cédula"
			status_update_message = f'Eu votei em {get_election_url(election)} - meu rastreador é {cast_vote.vote_hash[:10]}.. #heliosvoting'
		else:
...
Seu voto foi removido da eleição "{election.name}".
...
"""
			...
			# log it
			election.append_log(f"Eleitor {voter.voter_type}/{voter.voter_id} e seu voto foi removido após o congelamento da eleição")

		elif election.frozen_at:
			# log it
			election.append_log(f"Eleitor {voter.voter_type}/{voter.voter_id} removido após o congelamento da eleição")
...
def trustee_upload_decryption(request, election, trustee_uuid):
	...
	if trustee.verify_decryption_proofs():
		...
		try:
			# send a note to admin
			election.admin.send_message(f"{election.name} - descriptografia parcial do integrante da Comissão Eleitoral", f"integrante {trustee.name} ({trustee.email}) fez a sua descriptografia parcial.")
		except:
			# ah well
			pass
...
def voters_load(request, election, system):
	...
	# para a página de seleção da categoria a ser importada
	if request.method == "GET":
		parameters = {
			'election': election,
			'plugin': plugin
	...
	# para carga dos eleitores da categoria
		...
		if form_type == 'single':
			...
			messages.success(request, 'Usuários cadastrados.')
		else:
			return None
		...
		if bool(request.POST.get('confirm_p', 0)):
			...
			if 'voters_file' in request.FILES:
				...
				try:
					...
					voters = []
					problems.append("o seu arquivo CSV não pôde ser processado. Por favor, verifique se é um arquivo CSV válido")

				# check if voter emails look like emails
				if False in [validate_email(v['email']) for v in voters]:
					problems.append("os dados não se parecem com e-mails válidos. Você tem certeza de que carregou um arquivo com o e-mail como segundo campo?")
