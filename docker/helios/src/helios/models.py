...
class Election(HeliosModel):
	...
	ELECTION_TYPES = (('election', 'Eleição'), ('referendum', 'Referendo'))
	...
	# restrição no número de cédulas que podem ser depositadas
	cast_limit = models.IntegerField(null=True)
	...
	@property
	def metadata(self):
		return {'help_email': self.help_email or 'help@heliosvoting.org', 'private_p': self.private_p,
	...
	def add_voters_content(self, content: str, origin, description=None):
		"""
		...
		new_voter_file = VoterFile(election=self, voter_file_content=content, origin=origin, description=description)
		new_voter_file.save()

		return new_voter_file
	
	def add_voters_loaded(self, content: str, description: str):
		output = self.add_voters_content(content, 'SYSTEM', description)
		self.append_log(ElectionLog.VOTERS_LOADED_FROM_SYSTEM)
		return output
	
	def add_voters_file(self, uploaded_file):
		output = self.add_voters_content(uploaded_file.read().decode(), 'FILE')
	...
	def voting_deadline(self):
		return max([x for x in [self.voting_extended_until, self.voting_ends_at] if x is not None])

	@property
	def issues_before_freeze(self):
		issues = []
		if self.questions == None or len(self.questions) == 0:
			issues.append({'type': 'questions', 'action': "adicionar questões à cédula"})

		trustees = Trustee.get_by_election(self)
		if len(trustees) == 0:
			issues.append({'type': 'trustees', 'action': "adicionar ao menos um integrante da comissão eleitoral"})

		for t in trustees:
			if t.public_key == None:
				issues.append({'type': 'trustee keypairs', 'action': f'integrante da comissão eleitoral {t.name} gerar um par de chaves'})

		if self.voter_set.count() == 0 and not self.openreg:
			issues.append({"type": "voters", "action": 'informar a lista de eleitores (ou tornar o registro aberto ao público)'})
	...
	def ready_for_decryption_combination(self):
		...
		"""
		return len(self.trustees_without_tally()) == 0
	
	def trustees_without_tally(self) -> List['Trustee']:
		return [t for t in Trustee.get_by_election(self) if not t.decryption_factors]
	...
	def set_eligibility(self):
		...
		if not self.openreg:
			...
			# nas eleições com eleitores pré-definidos, caso não haja nenhum meio de autenticação, utiliza todas as opções habilitadas na configuração
			if len(auth_systems) == 0:
				auth_systems = copy.copy(settings.AUTH_ENABLED_AUTH_SYSTEMS)
	...
	def freeze(self):
		...
		if len(self.issues_before_freeze) > 0:
			raise Exception("não é possível congelar uma eleição com pendências")
	...
	@classmethod
	def winners_cut(cls, question, result, num_cast_votes, num_voters):
		
		# formata os dados do resultado e faz a ordenação
		result = [
			{
				'answer': x[0],
				'votes': x[1]
		...
		# condição para ser considerado vencedor. O padrão é sem restrição, utilizado no modo relativo.
		condition = lambda x: True
		if question['result_type'] == 'absolute':
			# maioria simples: maioria entre os votantes (metade dos votantes + 1)
			condition = lambda x: x['votes'] >= ((num_cast_votes // 2) + 1)
		elif question['result_type'] == 'absolute_max':
			# maioria absoluta: maioria entre os eleitores possíveis (metade dos eleitores + 1)
			condition = lambda x: x['votes'] >= ((num_voters // 2) + 1)
		else:
			# relativo: usa o padrão
			pass
		
		# determina o número de vencedores (1 ou o número máximo de votos possível para a questão)
		nr_of_winners = question['max'] or 1
		
		# determina o valor de corte (votos) para as opções
		cut = None
		for i in range(min(len(result), nr_of_winners)):
			if condition(result[i]):
				cut = result[i]['votes']
		
		# verifica se existe empate no valor de corte
		tie = False
		if cut is not None:
			tie = len([x for x in result if (x['votes'] >= cut) and condition(x)]) > nr_of_winners
	...
	def pretty_result(self):
		# usado para determinar o status da resposta
		def get_status(count, cut, tie):
			if cut is None:
				return 'loser'
			
			if (count == cut) and tie:
				return 'tie'
			
			return 'winner' if count >= cut else 'loser'
		...
		for i in range(len(self.questions)):
			...
			# determina o valor de corte e se houve empate no limite deste valor
			tie, cut = self.winners_cut(q, r, self.num_cast_votes, self.num_voters)
			
			pretty_question = []
			
			# para cada opção, indica o número de votos e seu status (vitória, derrota ou empate)
			for j in range(len(q['answers'])):
				a = q['answers'][j]
				count = self.result[i][j]
				pretty_question.append({
					'answer': a,
					'count': count,
					'status': get_status(count, cut, tie)
				})
			
			# lista o resultado em ordem decrescente (utilizando também a resposta para manter sempre a mesma ordem)
			# aqui é utilizado um artifício para ordernar o número de votos em forma negativa.
			# Desta forma, é possível ordenar os votos de forma decrescente, enquanto, a ordenação pelo nome é crescente.
			pretty_question.sort(key=lambda x: (-x['count'], x['answer']))
...
class ElectionLog(models.Model):
	...
	VOTERS_LOADED_FROM_SYSTEM = "voters loaded from system"
...
class VoterFile(models.Model):
	...
	ORIGIN_TYPES = (('FILE', 'Arquivo CSV'), ('SYSTEM', 'Sistema'))
	origin = models.CharField(max_length=250, null=False, choices=ORIGIN_TYPES)
	description = models.CharField(max_length=250, null=True)
	
	@property
	def pretty_origin(self):
		return dict(self.ORIGIN_TYPES)[self.origin]
	...
	def process(self):
		...
		for voter in self.itervoters():
			...
			if not existing_voter:
				...
				# caso exista um usuário com o mesmo login do eleitor, faz a associação entre os dois
				existing_user = User.get_by_id(voter['voter_id'])
				if existing_user:
					existing_voter.user = existing_user
...
class Voter(HeliosModel):
	...
	def generate_password(self, length=10):
		if self.voter_password:
			raise Exception("senha já existe")
...
class CastVote(HeliosModel):
	...
	def verify_and_store(self):
		...
		if self.is_quarantined:
			raise Exception("lançamento de votos está em quarentena, verificação e armazenamento estão postergados.")
	...
	def issues(self, election):
		...
		if self.vote.election_uuid != election.uuid:
			issues.append("O UUID de eleição do voto não coincide com a eleição para a qual este voto está sendo lançado")
