...
#
# Is deprecated: https://www.dlitz.net/software/pycrypto/
...
class RandomPool(object):
	...
	def _randomize(self, N=0, devname='/dev/urandom'):
		...
		data = b('')
		if N <= 0:
			nbytes = int(old_div((self.bits - self.entropy), 8) + 0.5)
		else:
			nbytes = int(old_div(N, 8) + 0.5)
		if winrandom:
	...
	def stir(self, s=''):
		...
		# back into the pool.
		for i in range(old_div(self.bytes, self._hash().digest_size)):
			h = self._hash(self._randpool)
			h.update((str(self.__counter) + str(i) + str(self._addPos) + s).encode('latin-1'))
	...
	def add_event(self, s=b('')):
		...
		delta = self._noise()
		s = (s + long_to_bytes(event) + bytes(4 * chr(0xaa).encode('utf8')) + long_to_bytes(delta))
		self._addBytes(s)
	...
	def _noise(self):
		...
		t = time.time()
		delta = old_div((t - self._lastcounter), self._ticksize * 1e6)
		self._lastcounter = t
	...
	def _measureTickSize(self):
		...
		interval.sort()
		self._ticksize = interval[old_div(len(interval), 2)]
