...
class EncryptedAnswer(HeliosObject):
	...
	def verify_plaintexts_and_randomness(self, pk):
		...
		# redo the encryption  # WORK HERE (paste from below encryption)
	...
	def toJSONDict(self, with_randomness=False):
		value = {'choices': [c.to_dict() for c in self.choices],
...
class EncryptedVote(HeliosObject):
	...
	def toJSONDict(self, with_randomness=False):
		return {'answers': [a.toJSONDict(with_randomness) for a in self.encrypted_answers],
				'election_hash': self.election_hash, 'election_uuid': self.election_uuid}
...
class Election(HeliosObject):
	FIELDS = ['uuid', 'questions', 'name', 'short_name', 'description', 'voters_hash', 'openreg', 'frozen_at',
			  'public_key', 'private_key', 'cast_url', 'result', 'result_proof', 'use_voter_aliases',
			  'voting_starts_at', 'voting_ends_at', 'election_type']

	JSON_FIELDS = ['uuid', 'questions', 'name', 'short_name', 'description', 'voters_hash', 'openreg', 'frozen_at',
