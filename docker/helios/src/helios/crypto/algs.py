...
class EGSecretKey(object):
	...
	def prove_decryption(self, ciphertext):
		...
		return m, {'commitment': {'A': str(a), 'B': str(b)}, 'challenge': str(c), 'response': str(t)}
...
class EGZKProof(object):
	...
	def to_dict(self):
		return {'commitment': {'A': str(self.commitment['A']), 'B': str(self.commitment['B'])},
				'challenge': str(self.challenge), 'response': str(self.response)}
