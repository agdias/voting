...
OAUTH_PARAMS = {'root_url': 'https://twitter.com', 'request_token_path': '/oauth/request_token',
				'authorize_path': '/oauth/authorize', 'authenticate_path': '/oauth/authenticate',
...
def get_user_info_after_auth(request):
	...
	return {'type': 'twitter', 'user_id': user_info['screen_name'], 'name': user_info['name'], 'info': user_info,
			'token': access_token}
...
def user_needs_intervention(user_id, user_info, token):
	...
	friendship = utils.from_json(twitter_client.oauth_request('http://api.twitter.com/1/friendships/exists.json',
...
def update_status(user_id, user_info, token, message):
	...
	"""
	twitter_client = _get_client_by_token(token)
	result = twitter_client.oauth_request('http://api.twitter.com/1/statuses/update.json', args={'status': message},
										  method='POST')
...
def send_notification(user_id, user_info, message):
	...
	result = twitter_client.oauth_request('http://api.twitter.com/1/direct_messages/new.json',
...
def follow_view(request):
	...
	if request.method == "POST":
		...
		if follow_p:
			...
			result = twitter_client.oauth_request('http://api.twitter.com/1/friendships/create.json',
												  args={'screen_name': USER_TO_FOLLOW}, method='POST')
