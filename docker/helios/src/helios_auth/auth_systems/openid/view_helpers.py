...
PAPE_POLICIES = ['AUTH_PHISHING_RESISTANT', 'AUTH_MULTI_FACTOR', 'AUTH_MULTI_FACTOR_PHYSICAL', ]

AX_REQUIRED_FIELDS = {'firstname': 'http://axschema.org/namePerson/first',
					  'lastname': 'http://axschema.org/namePerson/last', 'fullname': 'http://axschema.org/namePerson',
...
def finish_openid(session, request_args, return_to):
	...
	if request_args:
		...
		results = {consumer.CANCEL: {'message': 'OpenID authentication cancelled.'},

				   consumer.FAILURE: {'error': 'OpenID authentication failed.'},

				   consumer.SUCCESS: {'url': response.getDisplayIdentifier(),
									  'sreg': sreg_response and list(sreg_response.items()), 'ax': ax_items}}
