...
""" % (uuid.uuid1(), timezone.now().isoformat(), ticket)
	...
	return {'user_id': values.get('mail', None), 'name': values.get('displayName', None),
			'category': values.get('employeeType', None)}
...
def get_user_info_special(ticket):
	...
	val_url = CAS_URL + "validate" + '?service=' + urllib.parse.quote(
		_get_service_url()) + '&ticket=' + urllib.parse.quote(ticket)
