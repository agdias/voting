...
OAUTH_PARAMS = {'root_url': 'https://api.linkedin.com/uas', 'request_token_path': '/oauth/requestToken',
...
def get_user_info_after_auth(request):
	...
	return {'type': 'linkedin', 'user_id': user_id, 'name': "%s %s" % (first_name, last_name), 'info': {},
			'token': access_token}
...
def update_status(user_id, user_info, token, message):
	...
	"""
	return  # twitter_client = _get_client_by_token(token)  # result = twitter_client.oauth_request('http://api.twitter.com/1/statuses/update.json', args={'status': message}, method='POST')
