...
class User(models.Model):
	...
	@classmethod
	def register_user_as_voter(cls, voter: 'LoadedVoter'):
		if not cls.get_by_id(voter.login):
			cls.update_or_create(voter.type, voter.login, voter.name, {'email': voter.email})
	...
	@classmethod
	def register_users_as_voters(cls, voters: List['LoadedVoter']):
		cadastrados = [f"{u.user_type}:{u.user_id}" for u in cls.objects.all()]
		cadastrar = [v.get_as_user() for v in voters if f"{v.type}:{v.login}" not in cadastrados]
		User.objects.bulk_create(cadastrar)
	...
	def _display_html(self, size):
		...
		return """<img class="%s-logo small-logo" src="/static/media/login-icons/%s.png" alt="%s" /> %s""" % (
