...
ADMINS = (
	('Usuario Teste', 'no-reply@tjpr.jus.br'),
)
...
if get_from_env('DATABASE_URL', None):
	...
	# require SSL
	DATABASES['default']['OPTIONS'] = {'sslmode': get_from_env('DATABASE_SSLMODE', 'require')}
...
STATIC_ROOT = get_from_env('STATIC_ROOT', 'static/')
...
# STATICFILES_DIRS = ["server_ui/static/", "helios_auth/static/", "helios/static/", "heliosverifier/"]
...
if (get_from_env('HSTS', '0') == '1'):
	STS = True  # we're using our own custom middleware now  # SECURE_HSTS_SECONDS = 31536000  # not doing subdomains for now cause that is not likely to be necessary and can screw things up.  # SECURE_HSTS_INCLUDE_SUBDOMAINS = True

SECURE_BROWSER_XSS_FILTER = True
SECURE_CONTENT_TYPE_NOSNIFF = True

# configura a sessão
SESSION_COOKIE_AGE = int(get_from_env('SESSION_COOKIE_AGE', '7200'))
# SESSION_EXPIRE_SECONDS = int(get_from_env('SESSION_EXPIRE_SECONDS', '10'))
# SESSION_EXPIRE_AFTER_LAST_ACTIVITY = (get_from_env('SESSION_EXPIRE_AFTER_LAST_ACTIVITY', '1') == '1')
# SESSION_TIMEOUT_REDIRECT = get_from_env('SESSION_TIMEOUT_REDIRECT', '/')
...
DEFAULT_FROM_EMAIL = get_from_env('DEFAULT_FROM_EMAIL', 'helios-voting@tjpr.jus.br')
...
MAIN_LOGO_URL = get_from_env('MAIN_LOGO_URL', '/static/media/logo.png')
...
# LDAP
AUTH_LDAP_SERVER_URI = get_from_env('LDAP_URL', "ldap://ldap.forumsys.com")
AUTH_LDAP_BIND_DN = get_from_env('LDAP_BIND_DN', "cn=read-only-admin,dc=example,dc=com")
AUTH_LDAP_BIND_PASSWORD = get_from_env('LDAP_BIND_PASSWORD', "password")
AUTH_LDAP_USER_SEARCH = LDAPSearch(
	get_from_env('LDAP_SEARCH_BASE_DN', "dc=example,dc=com"),
	ldap.SCOPE_SUBTREE, get_from_env('LDAP_SEARCH_FILTER', "(uid=%(user)s)")
)
...
KEYCLOAK_SERVER_URL = get_from_env('KEYCLOAK_SERVER_URL', None)
KEYCLOAK_REALM = get_from_env('KEYCLOAK_REALM', None)
KEYCLOAK_CLIENT_ID = get_from_env('KEYCLOAK_CLIENT_ID', None)
KEYCLOAK_CLIENT_SECRET = get_from_env('KEYCLOAK_CLIENT_SECRET', None)
...
if ROLLBAR_ACCESS_TOKEN:
	...
	ROLLBAR = {'access_token': ROLLBAR_ACCESS_TOKEN, 'environment': 'development' if DEBUG else 'production', }
